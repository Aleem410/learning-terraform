...
resource "aws_spot_instance_request" "lab2_instance" {
  ...
  user_data = <<-EOF
      ...
      sudo echo "<html><body><h1>Hello this is practice-2 at instance id `curl http://169.254.169.254/latest/meta-data/instance-id` </h1></body></html>" > /var/www/html/index.html
      EOF
